...
class HEBO(AbstractOptimizer):
    ...
    def __init__(
        ...
        aux_input_constraints = []
        if input_constraints is None:
            input_constraints = []
        for input_constraint in input_constraints:
            if isinstance(input_constraint, str):
                split_input_constraint = input_constraint.split(" ")
                new_constr = []
                for el in split_input_constraint:
                    if el in self.space.para_names:
                        el = f"d['{el}']"
                    new_constr.append(el)
                new_constr = " ".join(new_constr)
            else:
                new_constr = input_constraint
            aux_input_constraints.append(new_constr)
        self.input_constraints = aux_input_constraints

    def quasi_sample(self, n: int, fix_input: Optional[Dict[str, float]] = None) -> pd.DataFrame:
        df_samps = None
        n_points = 0
        trials = 0
        while trials < 10 * (n + 1) and n_points < n:
            trials += 1
            samp = self.sobol.draw(1)
            ...
            for i, num_name in enumerate(self.space.numeric_names):
                if self.space.paras[num_name].is_discrete_after_transform:
                    x[:, i] = x[:, i].round()
            ...
            if df_samps is None:
                ...
                df_samps = pd.concat([df_samps, df_samp], ignore_index=True)
            n_points += 1
        assert len(df_samps) == n, df_samps
        return df_samps
